 ...
 */
fun EditText.makeClearableEditTextPass(
    onIsNotEmpty: (() -> Unit)?,
    onCanceled: (() -> Unit)?,
    ...
    val updateRightDrawable = {
        this.setCompoundDrawables(null, null,
            if (text.isNotEmpty() && this.compoundDrawables[2] == null) clearDrawable else if(text.isEmpty()) null else this.compoundDrawables[2],
            null)
...
fun EditText.makeClearableEditTextPass(onIsNotEmpty: (() -> Unit)?, onCanceled: (() -> Unit)?) {
    compoundDrawables[COMPOUND_DRAWABLE_RIGHT_INDEX]?.let { clearDrawable ->
        makeClearableEditTextPass(onIsNotEmpty, onCanceled, clearDrawable)
    }
}
